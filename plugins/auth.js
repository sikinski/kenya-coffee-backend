...
export default fp(async function (fastify, opts) {
    ...
    // Опциональная аутентификация - проверяет токен, но не возвращает ошибку если его нет
    fastify.decorate("optionalAuthenticate", async function (request, reply) {
        try {
            ...
            if (authHeader) {
                ...
                if (token) {
                    const payload = jwt.verify(token, process.env.ACCESS_SECRET);
                    request.user = payload; // добавляем user в запрос если токен валидный
                }
            }
        } catch (err) {
            // Игнорируем ошибки - просто не устанавливаем request.user
        }
    });
});
